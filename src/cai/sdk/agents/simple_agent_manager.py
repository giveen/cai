...
class SimpleAgentManager(AgentManager):
    ...
    def switch_to_single_agent(self, agent, agent_name: str):
        ...
        if agent_name not in self._message_history:
            # Use transfer_history if provided otherwise start empty
            self._message_history[agent_name] = transfer_history if transfer_history else []
        else:
            # Agent already has a history entry; override if transfer explicitly provided
            if transfer_history:
        ...
        try:
            if agent and hasattr(agent, "model") and hasattr(agent.model, "message_history"):
                ...
                if mgr_hist is not None:
                    try:
                        # Prefer sharing the same list reference so both stay in sync
                        agent.model.message_history = mgr_hist
                    except Exception:
                        # Fall back to clearing/extending if direct assignment isn't allowed
                        try:
                            agent.model.message_history.clear()
                            agent.model.message_history.extend(mgr_hist)
                        except Exception:
                            pass
        except Exception:
            pass
