...
def _configure_workspace_env(workspace_flag: Optional[str]) -> None:
    ...
    os.environ["CAI_WORKSPACE_ACTIVE_ROOT"] = str(workspace_root)
    os.environ["WORKSPACE_ROOT"] = str(workspace_root)

    if "CAI_WORKSPACE" not in os.environ:
        os.environ["CAI_WORKSPACE"] = workspace_root.name
    if "CAI_WORKSPACE_DIR" not in os.environ:
        os.environ["CAI_WORKSPACE_DIR"] = str(workspace_root.parent)


def _apply_global_flags(opts: GlobalOptions) -> None:
    if opts.silent:
        os.environ["CAI_SILENT"] = "1"
    if opts.debug:
...
def _bootstrap_config_and_env() -> BootstrapState:
    ...
    license_value = os.getenv("CAI_LICENSE_KEY", "").strip()
    if not license_value:
        try:
            cfg_license, _tier = CONFIG_STORE.resolve("CAI_LICENSE_KEY")
...
def _ensure_runtime_prerequisites(state: BootstrapState, command: str) -> None:
    ...
    raise CLIUserError(
        ...
        "or configure CAI_LICENSE_KEY before running interactive workloads."
    )


def _log_support_error(exc: BaseException) -> str:
    support_id = f"SUP-{uuid.uuid4().hex[:12].upper()}"
    workspace_root = Path(os.getenv("CAI_WORKSPACE_ACTIVE_ROOT") or os.getenv("WORKSPACE_ROOT") or os.getcwd()).resolve()
...
async def _dispatch_doctor(as_json: bool) -> int:
    ...
    table = Table(title="Cerebro Doctor Report")
