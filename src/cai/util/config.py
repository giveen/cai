"""Centralized immutable configuration for Cerebro-AI.
...
class CerebroConfig(BaseSettings):
    """Universal immutable configuration handler for Cerebro-AI.
    ...
    workspace_root: Path = Field(
        ...
        validation_alias=AliasChoices("CIR_WORKSPACE", "CAI_WORKSPACE_ROOT"),
    )
    total_system_ram_gb: float = Field(
        default_factory=_detect_total_system_ram_gb,
        validation_alias=AliasChoices("CAI_TOTAL_SYSTEM_RAM_GB", "CAI_RAM_TOTAL_GB"),
    )
    ...
    use_gpu_acceleration: bool = Field(
        default=True,
        validation_alias=AliasChoices("CAI_USE_GPU_ACCELERATION", "USE_GPU_ACCELERATION"),
    )
    cuda_device: str = Field(default="cuda:0", validation_alias=AliasChoices("CAI_CUDA_DEVICE", "CUDA_DEVICE"))
    cuda_visible_devices: str = Field(
        default=os.getenv("CUDA_VISIBLE_DEVICES", "0"),
        validation_alias=AliasChoices("CUDA_VISIBLE_DEVICES", "CAI_CUDA_VISIBLE_DEVICES"),
    )
    ...
    log_level: str = Field(default="INFO", validation_alias=AliasChoices("CAI_LOG_LEVEL", "LOG_LEVEL"))
    ...
    def as_environment(self) -> dict[str, str]:
        ...
        return {
            ...
            "CUDA_VISIBLE_DEVICES": self.cuda_visible_devices,
            "CAI_USE_GPU_ACCELERATION": str(self.use_gpu_acceleration).lower(),
            "CAI_TOTAL_SYSTEM_RAM_GB": str(self.total_system_ram_gb),
            "CAI_LOG_LEVEL": self.log_level,
        }
