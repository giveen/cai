"""Cerebro-AI performance and safety utility core.
...
class CostTracker:
    ...
    def _pricing_files(self) -> list[Path]:
        ...
        return [Path.cwd() / "pricing.json", repo_root / "pricing.json"]
    ...
    def check_price_limit(self, estimated_cost: float) -> None:
        ...
        raw_limit = os.getenv("CAI_MAX_USD", "").strip()
        if not raw_limit:
            return
        try:
...
def get_model_input_tokens(model: Any) -> int:
    ...
    if any(tag in model_name for tag in ("gpt-4o", "gpt-4.1", "claude-3", "o3", "o4")):
        return 128000
    return int(os.getenv("CAI_DEFAULT_CONTEXT_WINDOW", "32768"))
...
def cli_print_tool_output(
    ...
    **_: Any,
    ...
    - `CAI_STREAM=true`: suppress repeated command keys for the whole session.
    - `CAI_STREAM=false`: suppress repeated command keys within 0.5 seconds.
    """
    ...
    streaming_enabled = os.getenv("CAI_STREAM", "false").strip().lower() == "true"
...
def start_claude_thinking_if_applicable(model_name: str, agent_name: str, counter: int) -> dict[str, Any] | None:
    ...
    if os.getenv("CAI_STREAM", "false").strip().lower() != "true":
        return None
