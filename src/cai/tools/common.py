    ...
    *,
    ...
    if process.stdout is None or process.stderr is None:
        ...
        return CommandExecutionResult(
            ...
            exit_code=None,
            stdout="",
            stderr="Subprocess streams were not initialized.",
            error={"code": "stream_init_failed", "message": "Subprocess streams unavailable"},
        )
    ...
    timed_out = False
    try:
        stdout_raw, stderr_raw = await asyncio.wait_for(process.communicate(), timeout=float(timeout_value))
        ...
        if process.returncode is None:
            ...
            with suppress(Exception):
                await process.wait()
        stdout_raw = b""
        stderr_raw = b"Execution timed out by policy"

    ended = datetime.now(tz=UTC)
    stdout_text = _redact((stdout_raw or b"").decode("utf-8", errors="replace"))
    stderr_text = _redact((stderr_raw or b"").decode("utf-8", errors="replace"))
    ...
    mapped = _map_exit_code(process.returncode, timed_out=timed_out)
    if mapped is None:
        return CommandExecutionResult(
            ok=True,
            status="completed",
...
async def _run_local_async(command: str, stdout: bool = False, timeout: int = 100, stream: bool = False, call_id: Optional[str] = None, tool_name: Optional[str] = None, workspace_dir: Optional[str] = None, custom_args: Optional[Mapping[str, Any]] = None) -> str:
...
async def _run_docker_async(command: str, container_id: str, stdout: bool = False, timeout: int = 100, stream: bool = False, call_id: Optional[str] = None, tool_name: Optional[str] = None, args: Optional[Mapping[str, Any]] = None) -> str:
...
async def run_command_async(command: str, ctf: Any = None, stdout: bool = False, async_mode: bool = False, session_id: Optional[str] = None, timeout: int = 100, stream: bool = False, call_id: Optional[str] = None, tool_name: Optional[str] = None, args: Optional[Mapping[str, Any]] = None) -> str:
    _ = async_mode
    if session_id:
    ...
    active_container = os.getenv("CAI_ACTIVE_CONTAINER", "").strip()
...
def run_command(command: str, ctf: Any = None, stdout: bool = False, async_mode: bool = False, session_id: Optional[str] = None, timeout: int = 100, stream: bool = False, call_id: Optional[str] = None, tool_name: Optional[str] = None, args: Optional[Mapping[str, Any]] = None) -> str:
    if session_id:
        return send_to_session(session_id, command)

    if async_mode:
        if os.getenv("CAI_ACTIVE_CONTAINER", ""):
            sid = create_shell_session(command, ctf=ctf, container_id=os.getenv("CAI_ACTIVE_CONTAINER"))
        else:
