...
class CerebroCurlTool:
    ...
    async def request(self, *, target: str, args: str = "", timeout: int = 30) -> Dict[str, Any]:
        ...
        with CLI_UTILS.managed_env_context(base_env=clean_env) as runtime_env:
            ...
            timed_out = False
            try:
                stdout_data, stderr_data = await asyncio.wait_for(process.communicate(), timeout=float(timeout + 2))
                ...
                try:
                    ...
                    process.kill()
                    await process.wait()
                stdout_data, stderr_data = b"000", b"request timed out"

        http_code = self._parse_status_code(stdout_data.decode("utf-8", errors="replace"))
        stderr_text = self._secure_subprocess.redact_text(stderr_data.decode("utf-8", errors="replace"), redactions)
        stderr_text = self._redact_text(stderr_text)
