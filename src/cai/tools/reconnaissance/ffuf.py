...
class CerebroFfufTool:
    ...
    async def _start_fuzz_async(
        ...
        *,
        ...
        asyncio.create_task(self._monitor_job(job_id, redactions))
    ...
    async def _monitor_job(self, job_id: str, redactions: Dict[str, str]) -> None:
        job = self._jobs[job_id]
        process = job.process
        if process is None or process.stderr is None:
            job.status = "failed"
            job.summary = self._error("process_error", "ffuf process streams unavailable")
            return
        ...
        async def _read_stderr() -> None:
            while True:
                ...
                if not line:
                    break
                text = self._secure_subprocess.redact_text(line.decode("utf-8", errors="replace"), redactions)
                text = self._redact_sensitive(text.rstrip("\r\n"))
        ...
        if return_code == 0:
            ...
            self._audit("ffuf_job_failed", {"job_id": job_id, "return_code": return_code})
    ...
    def _resolve_politeness_rate(self, explicit: Optional[Union[int, float, str]]) -> float:
        if explicit is not None and str(explicit).strip():
            try:
                ...
                pass
        for key in ("CAI_FFUF_POLITENESS_RPS", "CAI_DISCOVERY_RPS", "CAI_SAFE_RPS"):
