        ...
        *,
        ...
        with CLI_UTILS.managed_env_context(base_env=clean_env) as runtime_env:
            try:
                ...
                state.status = "failed"
                state.error = {"code": "missing_dependency", "message": "wget binary not found on PATH"}
                return
            ...
            async def _read_stderr() -> None:
                ...
                while True:
                    ...
                    if not line:
                        break
                    text = self._secure.redact_text(line.decode("utf-8", errors="replace").strip(), redactions)
                    if text:
                        stderr_lines.append(text)
                    self._update_progress_from_line(state, text)
        ...
        state.progress_percentage = 100.0 if process.returncode == 0 and not timed_out and state.error is None else state.progress_percentage
        state.downloaded_bytes, state.file_count = self._measure_tree(state.work_dir)
        if process.returncode == 0 and state.error is None:
