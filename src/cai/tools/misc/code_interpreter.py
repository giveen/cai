"""Hardened Python sandbox tool for Cerebro-AI."""
...
class CerebroInterpreter:
    ...
    async def _run_in_ephemeral_docker(
        ...
        *,
        ...
        image = os.getenv("CAI_INTERPRETER_DOCKER_IMAGE", "python:3.12-slim")
    ...
    def _is_high_isolation_mode(self) -> bool:
        env_flag = os.getenv("CAI_INTERPRETER_HIGH_ISOLATION", "").strip().lower()
        if env_flag in {"1", "true", "yes", "on"}:
            return True
        try:
            resolved, _tier = CONFIG_STORE.resolve("CAI_INTERPRETER_HIGH_ISOLATION")
