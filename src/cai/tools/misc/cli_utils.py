"""Hardened CLI execution gateway for Cerebro-AI."""
    ...
    async def execute_command(
        ...
        *,
        ...
        with self.managed_env_context(base_env=clean_env) as runtime_env:
            ...
            timed_out = False
            try:
                raw_stdout, raw_stderr = await asyncio.wait_for(process.communicate(), timeout=float(timeout_seconds))
                ...
                try:
                    ...
                    process.kill()
                    await process.wait()
                raw_stdout, raw_stderr = b"", b"Execution timed out by policy.\n"

        exit_code = process.returncode
        stdout_text = self._secure_subprocess.redact_text(raw_stdout.decode("utf-8", errors="replace"), redaction_map)
        stderr_text = self._secure_subprocess.redact_text(raw_stderr.decode("utf-8", errors="replace"), redaction_map)

        stdout_clean = self.strip_ansi(stdout_text)
        stderr_clean = self.strip_ansi(stderr_text)
