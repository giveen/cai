...
class CerebroRAGTool:
    ...
    def _choose_embedding_provider(self) -> str:
        ...
        requested = os.getenv("CAI_RAG_EMBEDDINGS_PROVIDER", "").strip().lower()
        ...
        if os.getenv("OPENAI_API_KEY", "").strip():
            return "openai"
        if os.getenv("CAI_OLLAMA_URL", "").strip() or os.getenv("OLLAMA_HOST", "").strip():
            return "ollama"
        return "local"

    def _is_privacy_mode(self) -> bool:
        env_flag = os.getenv("CAI_PRIVACY_MODE", "").strip().lower()
        if env_flag in {"1", "true", "yes", "on"}:
            return True
        try:
            value, _tier = CONFIG_STORE.resolve("CAI_PRIVACY_MODE")
    ...
    async def _embed_openai(self, text: str) -> Optional[List[float]]:
        ...
        model = os.getenv("CAI_RAG_OPENAI_EMBED_MODEL", "text-embedding-3-small")
    ...
    async def _embed_ollama(self, text: str) -> Optional[List[float]]:
        base = os.getenv("CAI_OLLAMA_URL", "").strip() or os.getenv("OLLAMA_HOST", "").strip() or "http://127.0.0.1:11434"
        endpoint = base.rstrip("/") + "/api/embeddings"
        model = os.getenv("CAI_RAG_OLLAMA_MODEL", "bge-small")
    ...
    async def _embed_hf_local(self, text: str) -> Optional[List[float]]:
        ...
        try:
            ...
            model_name = os.getenv("CAI_RAG_HF_MODEL", "BAAI/bge-small-en-v1.5")
