...
def _resolve_terminal_id(token_info: dict, model) -> None:
    """Populate ``terminal_id`` (and ``terminal_number``) in *token_info* in-place."""
    # Priority 1: streaming context attached to the model
    ...
    # Priority 2: async execution context
    try:
    ...
    # Priority 3: thread-local tracking
    try:
...
def _build_token_info(model) -> dict:
    """Build the base token-info dict from a model instance."""
    ...
    _resolve_terminal_id(token_info, model)
    return token_info
...
def _get_agent_token_info():
    ...
    try:
        ...
        if model:
            return enrich_token_info_for_pricing(_build_token_info(model))

        # Fallback: most recent instance in the registry
