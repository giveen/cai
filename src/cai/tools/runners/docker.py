...
class CerebroDockerTool:
    ...
    def __init__(self) -> None:
        ...
        self._network_name = self._config_get("CAI_DOCKER_INTERNAL_NETWORK", _DEFAULT_NETWORK) or _DEFAULT_NETWORK
    ...
    def _create_for_compat(self, *, args: Any) -> Dict[str, Any]:
        ...
        image = str(payload.get("image") or self._config_get("CAI_DOCKER_DEFAULT_IMAGE", "alpine:latest"))
        ...
        *,
        ...
        while True:
            ...
            if update_streaming and tool_name and stream_call_id and (time.time() - last_update) >= 1.0:
                update_streaming(tool_name, tool_args, "".join(stdout_chunks + stderr_chunks), stream_call_id, token_info)
                last_update = time.time()
        ...
        if finish_streaming and tool_name and stream_call_id:
            finish_streaming(tool_name, tool_args, (stdout_text + ("\n" + stderr_text if stderr_text else "")).strip(), stream_call_id, execution_info, token_info)
        self._audit(
            "exec_completed",
        ...
        *,
        ...
        try:
            for item in client.api.exec_start(exec_id, stream=True, demux=True):
                out_chunk, err_chunk = item if isinstance(item, tuple) else (item, None)
                if out_chunk:
                    stdout_chunks.append(out_chunk.decode("utf-8", errors="replace"))
                if err_chunk:
                    stderr_chunks.append(err_chunk.decode("utf-8", errors="replace"))
    ...
    def _ttl_seconds(self, candidate: Optional[int]) -> int:
        raw = candidate if candidate is not None else self._config_int("CAI_DOCKER_TTL_SECONDS", _DEFAULT_TTL_SECONDS)
        return max(30, int(raw or _DEFAULT_TTL_SECONDS))

    def _mem_limit_mb(self, candidate: Optional[int]) -> int:
        raw = candidate if candidate is not None else self._config_int("CAI_DOCKER_MEM_LIMIT_MB", _DEFAULT_MEM_LIMIT_MB)
        return max(64, min(_MAX_MEM_LIMIT_MB, int(raw or _DEFAULT_MEM_LIMIT_MB)))

    def _nano_cpus(self, candidate: Optional[int]) -> int:
        raw = candidate if candidate is not None else self._config_int("CAI_DOCKER_NANO_CPUS", _DEFAULT_NANO_CPUS)
        return max(100_000_000, min(_MAX_NANO_CPUS, int(raw or _DEFAULT_NANO_CPUS)))

    def _safe_images(self) -> List[str]:
        raw = self._config_get("CAI_SAFE_DOCKER_IMAGES") or self._config_get("CAI_DOCKER_SAFE_IMAGES")
