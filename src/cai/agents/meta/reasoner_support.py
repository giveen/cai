"""
Cerebro Cognitive Load Balancer (CCLB)

Replaces the legacy reasoner_support.py thin wrapper with a full cognitive
state-management engine for the Cerebro-AI multi-agent suite.
...
class CerebroCognitiveSupport:
    ...
    the Cerebro-AI multi-agent suite.
    ...
    def build_critique_context(
        ...
        *,
        ...
        ``CritiqueTurn`` that the agent can pass to ``REASONING_TOOL.reason``
        as ``context``.
        """
        ...
        # Optionally invoke REASONING_TOOL if available
        if _REASONING_AVAILABLE and REASONING_TOOL is not None:
            try:
                critique = REASONING_TOOL.reason(
                    mode=MODE_CRITIQUE,
                    objective="Identify root cause of agent failure and suggest corrective action",
                    context=cognitive_note,
                    prior_output=f"Failed action: {failed_action}\nError: {observed_error}",
                    options=[
                        "Adjust confidence threshold",
                        "Pivot to alternative branch",
                        "Escalate to human operator",
                        "Retry with modified parameters",
                    ],
                    fetch_facts=True,
                    fact_query=f"failure pattern: {observed_error[:120]}",
                )
                cognitive_note += f"\nMODE_CRITIQUE summary: {json.dumps(critique, default=str)[:400]}"
            except Exception as exc:
                cognitive_note += f"\nMODE_CRITIQUE invocation failed: {exc}"
...
def create_reasoner_agent(
    ...
    agent_instructions = instructions if instructions is not None else create_system_prompt_renderer(default_instructions)
