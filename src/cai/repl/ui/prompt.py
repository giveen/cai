"""Cerebro-AI REPL prompt HUD engine.
...
class CerebroPromptHUD:
        ...
        if not self._ansi_safe:
            return [
                (
                    ...
                    f"{state.model} {state.agent_status} {'... ' if state.multiline else '> '}",
                )
            ]

        palette = self._palette_for(state)
        prefix = "..." if state.multiline else "cai"
    ...
    def _resolve_workspace_name(self) -> str:
        val = self._cfg_get("CAI_WORKSPACE")
        if val:
            return val
        try:
    ...
    def _resolve_target_name(self) -> str:
        return self._cfg_get("CAI_TARGET") or self._cfg_get("CTF_IP") or ""
    ...
    def _resolve_cost_limit(self) -> float:
        raw = self._cfg_get("CAI_PRICE_LIMIT")
        if not raw:
            return 0.0
        try:
            return float(raw)
    ...
    def _resolve_agent_status(self) -> str:
        if self._agent_status_provider is not None:
            try:
                ...
                pass
        state = self._cfg_get("CAI_AGENT_STATUS")
        if state:
            return state
        return "Thinking" if self._cfg_get("CAI_STREAM", "false").lower() == "true" else "Idle"
