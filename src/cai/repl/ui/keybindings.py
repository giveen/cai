"""Cerebro-AI productivity-first keybinding engine for the REPL."""
...
class CerebroKeyRegistry:
    ...
    def _load_binding_overrides(self) -> Dict[str, Any]:
        raw = CONFIG_STORE.get("CAI_KEYBINDINGS")
        if not raw or raw == "Not set":
            raw = os.getenv("CAI_KEYBINDINGS")
        if not raw:
            return {}
        ...
        try:
            ...
            self._key_log("Key Log: invalid CAI_KEYBINDINGS payload; using defaults")
            return {}
    ...
    async def _cycle_workspace(self, _event: KeyPressEvent) -> None:
        ...
        current = os.getenv("CAI_WORKSPACE", "")
    ...
    def _discover_models(self) -> List[str]:
        custom = CONFIG_STORE.get("CAI_MODEL_CYCLE") or os.getenv("CAI_MODEL_CYCLE", "")
...
def create_key_bindings(
    ...
    """Build REPL key bindings with Cerebro-AI productivity defaults.
