        ...
        if response.status_code == 200:
            ...
            try:
                ...
                ollama_response = requests.get(
                    f"{ollama_api_base.replace('/v1', '')}/api/tags", headers=headers, timeout=1
                )
...
def count_tools():
    ...
    try:
        ...
        tool_files = [
            f for f in tool_files if not f.endswith("__init__.py") and not f.endswith("__pycache__")
        ]
...
def display_framework_capabilities(console: Console):
    ...
    table.add_row(
        "Agents", str(count_agents()), "Specialized AI agents for different cybersecurity tasks"
    )

    # Add the table to a panel for better visual separation
    capabilities_panel = Panel(
        table, title="[bold blue]CAI Features[/bold blue]", border_style="blue", padding=(1, 2)
    )
...
def display_quick_guide(console: Console):
    ...
    help_text = Text.assemble(
        ...
        " (/a)\n",
        ("  CAI>/agent list", "green"),
        " - List all available agents\n",
        ("  CAI>/agent select [NAME]", "green"),
        " - Switch to specific agent\n",
        ("  CAI>/agent info [NAME]", "green"),
        " - Show agent details\n",
        ("  CAI>/parallel add [NAME]", "green"),
        " - Configure parallel agents\n\n",
        ("MEMORY & HISTORY", "bold yellow"),
        "\n",
        ("  CAI>/memory list", "green"),
        " - List saved memories\n",
        ("  CAI>/history", "green"),
        ...
        "\n",
        ("  CAI>/workspace set [NAME]", "green"),
        " - Set workspace directory\n",
        ("  CAI>/config", "green"),
        " - Manage environment variables\n",
        ("  CAI>/virt run [IMAGE]", "green"),
        " - Run Docker containers\n\n",
        ("TOOLS & INTEGRATION", "bold yellow"),
        "\n",
        ("  CAI>/mcp load [TYPE] [CONFIG]", "green"),
        " - Load MCP servers\n",
        ("  CAI>/shell [COMMAND]", "green"),
        " or $ - Execute shell commands\n",
        ("  CAI>/model [NAME]", "green"),
        " - Change AI model\n\n",
    ...
    config_text = Text.assemble(
        ...
        ("  CAI_MODEL", "green"),
        f" = {current_model}\n",
        ("  CAI_AGENT_TYPE", "green"),
        f" = {current_agent_type}\n",
        ("  CAI_PARALLEL", "green"),
        f" = {os.getenv('CAI_PARALLEL', '1')}\n",
        ("  CAI_STREAM", "green"),
        f" = {os.getenv('CAI_STREAM', 'true')}\n",
        ("  CAI_WORKSPACE", "green"),
    ...
    context_tip = Panel(
        Text.assemble(
            ...
            (
                "https://news.aliasrobotics.com/alias1-a-privacy-first-cybersecurity-ai/",
                "blue underline",
            ),
        ),
    ...
    console.print(
        Panel(
            Columns(
                [help_text, config_text, tips_group], column_first=True, expand=True, align="center"
            ),
