"""Case-isolated workspace command for CAI REPL."""
...
class CaseManager:
    ...
    def __init__(self, *, user: str, memory: MemoryManager) -> None:
        self._user = user
        self._memory = memory
        self._base = Path(os.getenv("CAI_WORKSPACE_DIR", str(Path.cwd() / "workspaces"))).expanduser().resolve()
    ...
    def _activate_case(self, case: CaseMetadata) -> None:
        ...
        os.environ["CAI_WORKSPACE"] = case.name
        os.environ["CAI_WORKSPACE_DIR"] = str(self._base)
        os.environ["CAI_WORKSPACE_ACTIVE_ROOT"] = str(root)
        os.environ["WORKSPACE_ROOT"] = str(root)
        os.environ["CAI_SHARED_ROOT"] = str(case.shared_dir)
        os.environ["CAI_PRIVATE_ROOT"] = str(case.private_dir)
        os.environ["CAI_AGENT_WRITE_ROOT"] = str(case.private_dir)
    ...
    def _env_keys() -> List[str]:
        return [
            "CAI_WORKSPACE",
            "CAI_WORKSPACE_DIR",
            "CAI_WORKSPACE_ACTIVE_ROOT",
            "WORKSPACE_ROOT",
            "CAI_SHARED_ROOT",
            "CAI_PRIVATE_ROOT",
            "CAI_AGENT_WRITE_ROOT",
        ]
