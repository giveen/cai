"""Model governance command for CAI REPL.
...
class LocalModelProvider(ModelProviderInterface):
    ...
    def _discover_vllm_models() -> List[str]:
        explicit = os.getenv("CAI_VLLM_MODELS", "")
...
class ModelOrchestrator:
    ...
    async def capability_audit(self, model_id: str) -> List[str]:
        ...
        ctx_env = os.getenv("CAI_CONTEXT_WINDOW") or os.getenv("CAI_MAX_CONTEXT")
    ...
    def _apply_runtime_selection(self, state: ModelSelectionState) -> None:
        os.environ["CAI_MODEL"] = state.selected_model
        os.environ["CAI_MODEL_PROVIDER"] = state.provider
    ...
    def _apply_runtime_parameters(params: Mapping[str, Any]) -> None:
        if "temperature" in params:
            os.environ["CAI_TEMP"] = str(params["temperature"])
        if "context_window" in params:
            os.environ["CAI_CONTEXT_WINDOW"] = str(params["context_window"])
        if "max_tokens" in params:
            os.environ["CAI_MAX_TOKENS"] = str(params["max_tokens"])
...
class ModelCommand(FrameworkCommand):
    """Model governance command with provider orchestration and audit controls."""

    name = "/model"
    description = "Select and govern LLM model/provider configuration"
    aliases = ["/mod"]
...
class ModelShowCommand(FrameworkCommand):
    """Compatibility command that maps to the new orchestrator catalog view."""

    name = "/model-show"
    description = "Show provider-agnostic model catalog"
    aliases = ["/mod-show"]

    @property
    def help(self) -> str:
        return "Usage: /model-show"
