"""Interactive onboarding bootstrapper for CAI REPL."""
...
class OnboardingManager:
    ...
    async def _configure_workspace(
        ...
        await self._invoke_command("config", ["set", "CAI_WORKSPACE_DIR", str(workspace_dir)])
        await self._invoke_command("/workspace", ["set", project_name])
        await self._invoke_command("config", ["set", "CAI_WORKSPACE", project_name, "--persist"])
        await self._invoke_command("config", ["set", "CAI_WORKSPACE_DIR", str(workspace_dir), "--persist"])
    ...
    async def _configure_provider(
        ...
        await self._invoke_command("config", ["set", "CAI_MODEL", model, "--persist"])
    ...
    async def _select_template(
        ...
        await self._invoke_command("config", ["set", "CAI_AGENT_TYPE", template.primary_agent, "--persist"])
...
class QuickstartCommand(FrameworkCommand):
    """Interactive environment bootstrapper for new CAI engagements."""
