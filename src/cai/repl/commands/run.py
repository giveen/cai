"""Supervised execution command for CAI REPL."""
...
class ExecutionSupervisor:
    ...
    def _ensure_registry_entry(self, agent: Any, requested_agent_name: Optional[str]) -> Optional[str]:
        ...
        model_name = getattr(getattr(agent, "model", None), "model", os.getenv("CAI_MODEL", "unknown"))
        ...
        agent_type = requested_agent_name or os.getenv("CAI_AGENT_TYPE", display_name.lower().replace(" ", "_"))
...
class RunCommand(FrameworkCommand):
    ...
    def help(self) -> str:
        ...
        return (
            ...
            f"Default max turns: {default_turns}. If CAI_MAX_TURNS is unset or unbounded, the supervisor enforces {default_turns} turns for safety.\n"
    ...
    def _resolve_agent(self, agent_name: Optional[str]) -> Any:
        ...
        requested_agent = agent_name or os.getenv("CAI_AGENT_TYPE", "one_tool")
        try:
    ...
    def _render_summary(self, summary: ExecutionImpactSummary, *, as_json: bool) -> None:
        ...
        table = Table(title="Supervised Run Summary")
