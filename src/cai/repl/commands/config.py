...
Manages three tiers of settings for the CAI framework:
...
Usage::
    ...
    /config show              → same
    /config get CAI_MODEL     → resolved value + tier source
    /config set CAI_MODEL gpt-4o
    /config set CAI_TEMP 0.7  → validated against schema
    /config reset CAI_MODEL   → remove session + user override
    /config source CAI_MODEL  → show which tier supplies the value
...
# ---------------------------------------------------------------------------
# System default entries (complete knowledge of all CAI env vars)
# ---------------------------------------------------------------------------
...
_SYSTEM_ENTRIES: List[ConfigEntry] = [
    ...
    ConfigEntry(name="CAI_BRIEF",        description="Enable brief output mode",           default="false",
                value_type="bool"),
    ConfigEntry(name="CAI_MAX_TURNS",    description="Maximum agent turns (inf = unlimited)", default="inf"),
    ConfigEntry(name="CAI_TRACING",      description="Enable OpenTelemetry tracing",       default="true",
                value_type="bool"),
    ConfigEntry(name="CAI_AGENT_TYPE",   description="Default agent type",                 default="one_tool"),
    ConfigEntry(name="CAI_STATE",        description="Enable stateful mode",               default="false",
                value_type="bool"),
    ConfigEntry(name="CAI_MEMORY",       description="Memory mode (false/episodic/semantic/all)", default="false",
                allowed_values=["false", "episodic", "semantic", "all", "False"]),
    ConfigEntry(name="CAI_MEMORY_ONLINE",         description="Enable online memory",  default="false",
                value_type="bool"),
    ConfigEntry(name="CAI_MEMORY_OFFLINE",        description="Enable offline memory", default="false",
                value_type="bool"),
    ConfigEntry(name="CAI_MEMORY_ONLINE_INTERVAL",description="Turns between online memory updates", default="5",
                value_type="int", min_val=1),
    ConfigEntry(name="CAI_ENV_CONTEXT",  description="Inject dirs/env into LLM context",  default="true",
                value_type="bool"),
    ConfigEntry(name="CAI_PRICE_LIMIT",  description="Session price limit in USD",         default="1",
                value_type="float", min_val=0.0),
    ConfigEntry(name="CAI_REPORT",       description="Reporter mode",                      default="ctf",
                allowed_values=["ctf", "nis2", "pentesting", "false"]),
    ConfigEntry(name="CAI_SUPPORT_MODEL",    description="Model for support agent",         default="o3-mini"),
    ConfigEntry(name="CAI_SUPPORT_INTERVAL", description="Turns between support agent runs",default="5",
                value_type="int", min_val=1),
    ConfigEntry(name="CAI_STREAM",       description="Enable streaming responses",          default="true",
                value_type="bool"),
    ConfigEntry(name="CAI_WORKSPACE",    description="Current workspace name",              default=None),
    ConfigEntry(name="CAI_WORKSPACE_DIR",description="Path to workspace directory",         default=None,
                value_type="path"),
    ConfigEntry(name="CAI_GUARDRAILS",   description="Enable prompt injection guardrails",  default="true",
                value_type="bool"),
    ConfigEntry(name="CAI_PARALLEL",     description="Number of parallel agent instances",  default="1",
                value_type="int", min_val=1),
    ConfigEntry(name="CAI_TEMP",         description="Default LLM temperature",             default="1.0",
...
def set_env_var(var_name: str, value: str) -> bool:
    ...
    ok, _ = CONFIG_STORE.set(var_name, value, tier=TIER_SESSION)
    return ok
...
class ConfigCommand(FrameworkCommand):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.add_subcommand("show",    "Display all settings (secrets masked)",      self._sub_show)
        self.add_subcommand("list",    "Alias for show",                             self._sub_show)
        self.add_subcommand("get",     "Get a setting's value and source tier",      self._sub_get)
        self.add_subcommand("set",     "Validate and apply a setting",               self._sub_set)
        self.add_subcommand("reset",   "Remove session + user overrides",            self._sub_reset)
        self.add_subcommand("source",  "Show full resolution chain for a setting",   self._sub_source)
        self.add_subcommand("encrypt", "Report encryption backend status",           self._sub_encrypt)
    ...
    async def execute(self, args: List[str]) -> bool:
        ...
        # Fallback: treat as a get by name (convenience: /config CAI_MODEL)
    ...
    def _sub_show(self, _args: Optional[List[str]] = None) -> bool:
        ...
        tbl.add_column("T",       style="cyan",    width=3,
                       header="T", no_wrap=True)
