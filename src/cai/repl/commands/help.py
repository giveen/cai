"""Dynamic help command for CAI REPL.
...
from cai.repl.commands.base import COMMAND_ALIASES, COMMANDS, FrameworkCommand, register_command
...
class DocGenerator:
    ...
    def build_examples(name: str) -> List[str]:
        ...
        specific = {
            ...
            "config": ["/config list", "/config set CAI_MODEL o4-mini"],
...
class HelpCommand(FrameworkCommand):
    """Registry-driven REPL documentation command."""

    name = "help"
    description = "Dynamic, risk-aware help for all active commands"
    aliases = ["/help", "/h", "/?"]
    ...
    def __init__(self) -> None:
        ...
        self.add_subcommand("commands", "List all available commands", self._sub_commands)
        self.add_subcommand("aliases", "Show command alias registry", self._sub_aliases)
        self.add_subcommand("quick", "Show quick command reference", self._sub_quick)
        self.add_subcommand("platform", "Show platform extension help", self._sub_platform)

        # Compatibility entry points expected by older integrations/tests.
        self.add_subcommand("memory", "Show memory command help", self._sub_memory)
        self.add_subcommand("agent", "Show agent command help", self._sub_agent)
        self.add_subcommand("graph", "Show graph command help", self._sub_graph)
        self.add_subcommand("shell", "Show shell command help", self._sub_shell)
        self.add_subcommand("env", "Show env command help", self._sub_env)
        self.add_subcommand("model", "Show model command help", self._sub_model)
        self.add_subcommand("turns", "Show turns control help", self._sub_turns)
        self.add_subcommand("config", "Show config command help", self._sub_config)
