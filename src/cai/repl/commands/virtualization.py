...
class SandboxManager:
    ...
    def _trusted_registries(self) -> List[str]:
        raw = os.getenv("CAI_TRUSTED_REGISTRIES", "").strip()
        if not raw:
            raw = CONFIG_STORE.get("CAI_TRUSTED_REGISTRIES")
    ...
    def _health_policy() -> HealthPolicy:
        try:
            max_cpu = float(os.getenv("CAI_SANDBOX_MAX_CPU_PERCENT", "90"))
        except ValueError:
            max_cpu = 90.0
        try:
            max_mem = float(os.getenv("CAI_SANDBOX_MAX_MEM_MB", "3072"))
        except ValueError:
            max_mem = 3072.0
...
class VirtualizationCommand(FrameworkCommand):
    ...
    def help(self) -> str:
        return (
            ...
            "  - Image registry must be trusted via CAI_TRUSTED_REGISTRIES.\n"
